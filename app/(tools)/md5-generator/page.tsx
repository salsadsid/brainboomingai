import ToolSeo from "@/components/seo/ToolSeo";
import ToolHeader from "@/components/tools/ToolHeader";
import { toolKeywords } from "@/config/constants";
import { buildMetadata } from "@/lib/seo";
import MD5GeneratorTool from "./MD5GeneratorTool";

export const metadata = buildMetadata({
  title: "Free MD5 Hash Generator - Text to MD5 Converter",
  description: "Generate MD5 hashes instantly with our free online tool. Useful for checksums and data integrity verification. Fast and easy to use.",
  path: "/md5-generator",
  keywords: toolKeywords("/md5-generator"),
});

export default function MD5GeneratorPage() {
  return (
    <>
      <ToolSeo path="/md5-generator" />
      <ToolHeader href="/md5-generator" />
      <MD5GeneratorTool />
    </>
  );
}
